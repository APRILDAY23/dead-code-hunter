  ...
  analyze(filePath: string, content: string) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (defMatch) {
        ...
        definitions.push({ name, kind: 'method', file: filePath, line: lineNum, column: 0, exported });
        continue;
      }
      ...
      if (moduleMatch) {
        definitions.push({ name: moduleMatch[1], kind: 'module', file: filePath, line: lineNum, column: 0, exported: true });
        continue;
      }
